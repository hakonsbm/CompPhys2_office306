...
\section{Methods}
	...
	\subsection{Blocking}
		...
		\begin{figure}
			...
			\protect\caption{STD vs. blocksize with $10^{5}$ MC cycles.  The STD should have a plateuing trend as the blocksize increases.} \label{fig01:std_Stuff}
	...
	\subsection{Verification of the model}
		...
		\subsubsection{Verification of the Padè-Jastrow part.}
			...
			Let us consider the gradient ratio of the Padè-Jastrow factor in Helium, \(\nabla\frac{\psi_C(\vb{r}_{12})}{\psi_C(\vb{r}_{12})}\) with \(\psi_C(\vb{r_{12}}) = e^{\frac{r_{12}}{2(1+\beta r_{12})}}\). Using the results from equation \eqref{eq:gradient_ratio_Jastrow} on Helium we get
			...
			Testing the that the program reproduces this for the helium atom indicates the \(\frac{\nabla \Psi_C}{\Psi_C}\) is being calculated correctly.
		...
		\subsubsection{Derivatives of single particle wavefunctions}
			...
			\begin{center}
				...
				\\ \hline
					...
					\( e^{- \alpha ri} \)
					&
					\( - \frac{\alpha}{r_{i}} \left(x_{i}\^{i} + y_{i}\^{j} + z_{i}\^{k}\right) e^{- \alpha r_{i}} \)
					&
				...
				\\	\hline
					...
					&
					\( \frac{\alpha e^{- \frac{\alpha r_{i}}{2}}}{4 r_{i}} \left(\alpha r_{i} - 4\right) \left(x_{i}\^{i} + y_{i}\^{j} + z_{i}\^{k}\right) \)
					&
				...
				\\	\hline
					...
					&
					\( - \frac{\alpha e^{- \frac{\alpha r_{i}}{2}}}{2 r_{i}} \left(\alpha x_{i}^{2}\^{i} + \alpha x_{i} y_{i}\^{j} + \alpha x_{i} z_{i}\^{k} - 2 r_{i}\right) \)
					&
				...
				\\	\hline
					...
					\( \alpha x_{i} e^{- \frac{\alpha r_{i}}{2}} \)
					&
					\( - \frac{\alpha e^{- \frac{\alpha r_{i}}{2}}}{2 r_{i}} \left(\alpha y_{i} x_{i}\^{i} + \alpha y_{i}^{2}\^{j} + \alpha y_{i} z_{i}\^{k} - 2 r_{i}\right) \)
					&
					\( \frac{\alpha^{2} x_{i}}{4 r_{i}} \left(\alpha r_{i} - 8\right) e^{- \frac{\alpha r_{i}}{2}} \)
				\\	\hline
					\(\psi_{2Pz}\)
					...
					\( \alpha x_{i} e^{- \frac{\alpha r_{i}}{2}} \)
					&
					\( - \frac{\alpha e^{- \frac{\alpha r_{i}}{2}}}{2 r_{i}} \left(\alpha z_{i} x_{i}\^{i} + \alpha z_{i} y_{i}\^{j} + \alpha z_{i}^{2}\^{k} - 2 r_{i}\right) \)
					&
					\( \frac{\alpha^{2} x_{i}}{4 r_{i}} \left(\alpha r_{i} - 8\right) e^{- \frac{\alpha r_{i}}{2}} \)
				\\ \toprule
				\end{tabular}
...
\section{Results and discussion}
	\begin{table}
		...
		energies found in research papers \cite{Koput:2011:PCCP} \cite{NEON}.}
	...
	For Neon, with $5 \times 10^{5}$ cycles we get an energy of $-146.958$ with a variance of $0.0261$.
	...
	for energy in the ground state of Neon to be $TODO$ \cite{NEON}.
	...
	\subsection{Speedup with MPI}
		...
		\center
				\begin{tabular}{| c | c| c| c| c|}
				    ...
				    \\ \hline
				    \textbf{Time to exec [s]}	&	1.0	&	1.97	&	2.90	&	3.35
				    \\	\hline
			  \end{tabular}
			  \caption{MPI speedup}
