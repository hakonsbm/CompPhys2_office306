...
\abstract{}
By using Variational Monte Carlo methods with importance sampling, blocking and MPI we find the ground state energies of Beryllium and Neon. We also calculate the onebody density with and without the Jastrow factor. SOME RESULTS
...
\section{Methods}
	...
	\subsection{Implementation of MPI}
		...
		Implementing MPI in the Monte Carlo method is very easy. As we deal with statistical values we can easily split up the problem. Each process will run its own set of samples. The number of samples used by each process is simply $n/p$, where $n$ is the total number of samples we want to do, and $p$ is the number of processes. In the end all processors send their results to the master process, which takes the average of all the values. 
